...
namespace Dears {
	...
	void Graphics::InitDepthStencilStates(ComPtr<ID3D11Device>& device)
	{
		...
		// Punching pass의 목적은 원본 모델이 차지하는 화면 영역을 stencil = 1로 표시하는 것이다.
		// 메인 패스가 이미 원본 모델의 depth를 써둔 뒤에 이 패스가 실행되므로,
		// 여기서 depth test를 다시 걸면 같은 표면인데도 정밀도 차이로 일부 픽셀이 실패할 수 있다.
		// 그 픽셀은 stencil이 비어 있게 되고, Edge pass의 주황색 확대 모델이 내부로 새어 들어온다.
		// 그래서 punching은 depth와 경쟁하지 않고 stencil mask만 만드는 전용 패스로 둔다.
		punchDsDesc.DepthEnable = false;
		punchDsDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
		punchDsDesc.DepthFunc = D3D11_COMPARISON_ALWAYS;
