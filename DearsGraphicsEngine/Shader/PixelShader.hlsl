...
float4 main(PixelShaderInput input) : SV_TARGET0
{
   ...
   // Scene Hierarchy에서 수집한 라이트만 순회한다.
   // 타입별 계산은 Common.hlsli에 모아 메시 종류와 관계없이 같은 규칙을 사용한다.
   [loop]
   for (uint lightIndex = 0; lightIndex < min(lightNum, (uint)MAX_LIGHTS); ++lightIndex)
   {
       if (lights[lightIndex].lightType == 1)
       {
           color += ComputeDirectionalLight(lights[lightIndex], material, input.normal, toEye);
       }
       else if (lights[lightIndex].lightType == 2)
       {
           color += ComputePointLight(lights[lightIndex], material, input.posWorld, input.normal, toEye);
       }
       else if (lights[lightIndex].lightType == 3)
       {
           color += ComputeSpotLight(lights[lightIndex], material, input.posWorld, input.normal, toEye);
       }
   }
