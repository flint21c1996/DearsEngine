...
{
	if (_viewDir.LengthSquared() < 0.0001f)
	{
		_viewDir = Vector3::UnitZ;
	}
	_viewDir.Normalize();
	mViewDir = _viewDir;

	// LookToLH는 direction과 up이 평행하면 안정적인 카메라 기저를 만들 수 없다.
	// Directional Light가 아래쪽(-Y)을 바라보는 경우가 흔하므로, 방향과 거의 평행한
	// 월드 Y축 대신 X축을 기준축으로 골라 Right/Up을 다시 직교화한다.
	const Vector3 referenceUp = std::abs(mViewDir.Dot(Vector3::UnitY)) > 0.95f
		? Vector3::UnitX
		: Vector3::UnitY;
	mRightDir = referenceUp.Cross(mViewDir);
	mRightDir.Normalize();
	mViewUp = mViewDir.Cross(mRightDir);
	mViewUp.Normalize();
}

void Camera::SetDirection(Vector3 viewDirection, Vector3 viewUp)
{
	...
	if (viewUp.LengthSquared() < 0.0001f)
	{
		viewUp = Vector3::UnitY;
	}
	viewUp.Normalize();

	// 오브젝트 회전행렬에서 온 Forward와 Up은 원래 서로 직교한다.
	// 그래도 부동소수점 오차가 누적될 수 있으므로 Gram-Schmidt 방식으로
	// Right를 만든 뒤 Up을 다시 계산해 완전한 직교 기저로 정리한다.
	Vector3 right = viewUp.Cross(viewDirection);
	if (right.LengthSquared() < 0.0001f)
	{
		// 잘못된 외부 입력으로 두 벡터가 평행할 때만 기존 안전 경로를 사용한다.
		SetDirection(viewDirection);
		return;
	}
	right.Normalize();

	mViewDir = viewDirection;
	mRightDir = right;
	mViewUp = mViewDir.Cross(mRightDir);
	mViewUp.Normalize();
}
...
{
	// 0에 가까운 크기는 유효한 투영행렬을 만들 수 없으므로 최소 범위를 보장한다.
	mOrthoGraphicScreenWidth = (std::max)(width, 0.1f);
	mOrthoGraphicScreenHeight = (std::max)(height, 0.1f);
	isPerspective = false;
}
